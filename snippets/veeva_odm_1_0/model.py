...
class EventGroupRef(OE.ODMElement):
    namespace = "vee"
    EventGroupOID = T.OID(required=True)


class Protocol(ODM.Protocol):
    Description = ODM.Protocol.Description
    EventGroupRef = T.ODMListObject(element_class=EventGroupRef, namespace="vee")
...
class EventGroupDef(OE.ODMElement):
    ...
    Repeating = T.ExtendedValidValues(required=False, valid_values=["Yes", "No"], namespace="vee")
